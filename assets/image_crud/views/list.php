<?php
	...
	$this->set_js('template/plugins/slimScroll/jquery.slimscroll.min.js');
?>
...
<div id='ajax-list'>
	...
	<?php foreach($photos as $photo_num => $photo){?>
			<li id="photos_<?php echo $photo->$primary_key; ?>" class = "photo_common">
				<div class='photo-box'>
					...
					<?php if(!$unset_delete){?><div class='delete-box'>
						<a href='<?php echo $photo->delete_url?>' class='delete-anchor' tabindex="-1">Xóa</a>
					</div><?php }?>
					<div class="clear"></div>
					<div <?php echo ($photo->status == 0)?'style = "visibility:hidden;"':'';?> <?php echo ($photo->status == 0)?'id = "default_gallery"':'';?>  class="alert-success"><i class="icon fa fa-check"></i><a href ="admin/gallery/changeDefaultGallery/<?php echo $photo->id;?>?redirect=<?php echo base64_encode(getCurrentUrl());?>" style = "color:white;">Mặc định!</a></div>
				</div>
			</li>
	<?php }?>
		</ul>
