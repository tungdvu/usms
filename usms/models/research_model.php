...
class Research_Model extends MY_Model
{
	...
	function addUser($data)
	{
			...
			$this->_add(PREFIX.'users', $data);
			return true;
	}

	//person
	function insertResearchPerson($data)
	{
			//$this->db->where('uid', $uid);
			$this->_add(PREFIX.'research_person', $data);
			return true;
	}

	function getPersonByResearch($research_id){
		$research_id = (int)$research_id;
		$this->db->select(PREFIX.'research_person.*,'.PREFIX.'users.fullname as name');
		$this->db->from(PREFIX.'research_person');
		$this->db->join(PREFIX.'users',''.PREFIX.'users.id = '.PREFIX.'research_person.uid','left');
		//$this->db->join(PREFIX.'research_person_out',''.PREFIX.'research_person.uid = '.PREFIX.'research_person_out.id','left');
	...
	function updatePerson($id, $data)
	{
		...
		{
			$this->db->where('id', $id);
			$this->db->update(PREFIX.'research_person', $data);
			return true;
		} else return false;
	}
	...
	function deletePersonById($id)
	{
		$id = (int)$id;
		$this->db->where('id', $id);
		$this->db->delete(PREFIX.'research_person');
	}
