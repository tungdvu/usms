        ...
        foreach ($post as $key => $value) {
          echo '<article class="item-list tie_lightbox">
                <h2 class="post-box-title">
                  <a  style = "text-decoration: none;" href="teacher.php/'.slug($value['user']).'-gv'.$value['user_id'].'/bao-cao-khoa-hoc/'.(isset($value['id'])?(slug($value['title']).'-a'.$value['id'].'.html'):null).'" rel="bookmark">'.(isset($value['title'])?$value['title']:'Bài Không Có Tiêu đề').'</a>
                </h2>
                <p class="post-meta">
                  ...
                  <a href="teacher.php/'.slug($value['user']).'-gv'.$value['user_id'].'/bao-cao-khoa-hoc/'.(isset($value['id'])?(slug($value['title']).'-a'.$value['id'].'.html'):null).'#comments">Bình luận</a></span>
                ...
                <div class="post-thumbnail" >
                  <a href="teacher.php/'.slug($value['user']).'-gv'.$value['user_id'].'/bao-cao-khoa-hoc/'.(isset($value['id'])?(slug($value['title']).'-a'.$value['id'].'.html'):null).'" rel="bookmark">
                ...
                <div class="entry">
                  ...
                  <a class="more-link" href="teacher.php/'.slug($value['user']).'-gv'.$value['user_id'].'/bao-cao-khoa-hoc/'.(isset($value['id'])?(slug($value['title']).'-a'.$value['id'].'.html'):null).'" >Read More »</a>
                  </div>
                  <div class="clear"></div>
                </article>';
        }
  ...
  if(isset($num_row)&&!empty($num_row)){
    echo '<div class="recent-box-pagination">
                <div class="pagination">
                  ...
                  <a '.(($page<=1)?"":('href="teacher.php/'.slug(isset($teacher['fullname'])?$teacher['fullname']:null).'-gv'.slug(isset($teacher['id'])?$teacher['id']:null).'/bao-cao-khoa-hoc.page'.($page+1).'.html"')).'">← Prev</a>
                  </span>';
                  ...
                  for ($i=$start; $i <= $end; $i++) { 
                    ...
                    else echo '<a class="page" href="teacher.php/'.slug(isset($teacher['fullname'])?$teacher['fullname']:null).'-gv'.slug(isset($teacher['id'])?$teacher['id']:null).'/bao-cao-khoa-hoc.page'.$i.'.html">'.$i.'</a>';
                  }

            echo '<span '.(($page>=($num_row/$post_in_page))?'class="disabled"':"").'>
                    <a '.(($page>=($num_row/$post_in_page))?"":('href="teacher.php/'.slug(isset($teacher['fullname'])?$teacher['fullname']:null).'-gv'.slug(isset($teacher['id'])?$teacher['id']:null).'/bao-cao-khoa-hoc.page'.($page+1).'.html"')).'">Next →</a>
                  </span>
                </div>
          </div>';
    }
